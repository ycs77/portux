...
> **random port 決策（2026-06）**：刻意採「從候選池**隨機抽**」而非競品的「下一個可用 port」。理由：portux 定位是「逛街挑黃金空位」，使用者要的是「幫我隨便挑一個乾淨的來用」，而非機械式遞增；遞增式（如 `get-port-cli`）會讓大家都擠在 3000、3001…，random 反而更分散、更貼合定位。候選池定義為 `占用=無 ∧ 常見標籤=無 ∧ port ≥ 1024`：排除占用是當然，排除常見預設是因為那些黃金 port 留著給人「指名挑選」、不該被隨機吃掉，排除 `<1024` 則因保留埠需 root 不適合當新專案開發埠。兩條輸出路徑共用同一候選池——TUI `r` 鍵在可見列中跳游標、`--random` flag 走非互動路徑直接吐一個數字（方便 `PORT=$(portux --random)` 取用）。
